...
for env in `env | sort | grep ^LIVY_CLIENT_ | sed 's/=.*//'`; do
    key=`echo ${env,,} | sed 's/^livy_client_/livy_/' | sed "s/_/./g" | sed 's/1\s*./\U&\E/g' | sed 's/0/-/g' | sed 's/1//g' | sed 's/2/_/g' `
...
sed -i -e '$a\' ${SPARK_DEFAULTS_CONF_FILE}
for env in `env | sort | grep ^LIVY_SPARK_ | sed 's/=.*//'`; do
    key=`echo ${env,,} | sed 's/^livy_//' | sed "s/_/./g" | sed 's/1\s*./\U&\E/g' | sed 's/0/-/g' | sed 's/1//g' | sed 's/2/_/g' `
...
readConfigs () {
  sourceDir=$1
  destFile=$2
  temp="spark.kubernetes.driverenv.kafka.host"
  echo "" >> ${destFile}
  echo "# Config mounted from ${sourceDir}" >> ${destFile}
  if [ -d "${sourceDir}" ]; then
    for key in $(ls ${sourceDir}); do
	  echo " key=$key  temp=$temp inside if condition" >>  ${destFile}
	  if [ $key == $temp ]; then
		key="spark.kubernetes.driverEnv.KAFKA_HOST"
		echo "inside if condition" >>  ${destFile}
	  fi
